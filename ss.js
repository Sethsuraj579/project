        ...
        <div className="max-w-screen-xl mx-auto p-4 sm:p-6 lg:p-8">
          ...
          <header className="flex items-center justify-between pb-4 border-b border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <h1 className=" App text-2xl font-bold text-gray-900">LegalSimplify AI</h1>
            </div>
          </header>

          <div className="mt-6">
              <div className="flex justify-between items-center mb-2">
                <h2 className="text-sm font-medium text-gray-600">Document Processing Progress</h2>
                <p className="text-xs text-gray-500 font-medium">1/4 Steps Complete</p>
              </div>
          ...
          <main className="grid grid-cols-1 lg:grid-cols-4 gap-8 mt-6">
            <aside className="lg:col-span-1">
              <div>
                <h3 className="text-lg font-semibold mb-4 text-gray-900">Document Sections</h3>
                <nav className="space-y-1">
                  {sidebarNavItems.map((item, index) => (
                    <a key={item.name} href="#!" className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${index === 0 ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100'}`}>
                      {item.icon}
            ...
            <section className="lg:col-span-3">
               {isLoading ? (
                  ...
                  <div className="p-8 bg-white rounded-lg border border-gray-200 space-y-6 fade-in">
                    ...
                    <p className="text-gray-600 leading-relaxed whitespace-pre-wrap">{simplifiedText}</p>
                  </div>
               ) : (
                  <div className="space-y-6">
                    <div className="text-center p-8">
                        <div className="inline-flex items-center justify-center h-16 w-16 rounded-full bg-indigo-50 text-indigo-500 mb-4">
                            <InfoIcon />
                        </div>
                        <h2 className="text-2xl font-bold text-gray-900">Upload Your Legal Document</h2>
                        <p className="text-base text-gray-500 mt-2">Drag and drop your legal document here, or click to browse.<br/>We support PDF, DOC, and DOCX formats.</p>
                    </div>
                    ...
                    <div className="relative flex py-2 items-center">
                        ...
                        <span className="flex-shrink mx-4 text-sm text-gray-400">Or paste your text directly</span>
                        <div className="flex-grow border-t border-gray-300"></div>
                    </div>
                    
                    <textarea value={pastedText} onChange={(e) => { setPastedText(e.target.value); setSelectedFile(null); }} placeholder="Paste your legal document text here..." className="w-full h-32 p-4 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none transition"></textarea>
